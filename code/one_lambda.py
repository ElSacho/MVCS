...
class OneLambdaPredictor:
    ...
    def fit(self, x_train, y_train, alpha,
        ...
        self.mat_A = torch.nn.parameter.Parameter(torch.randn(k, k), requires_grad=True)
        ...
        optimizer = torch.optim.Adam([
            ...
            {'params': self.mat_A, 'lr': lr_matrix_model},  # Learning rate for self.matrix_model
        ...
        last_lambdas_weight = self.mat_A
        last_q = self.q.item()
        ...
        for epoch in range(num_epochs_our_loss):
            for x, y in dataloader:
                ...
                if torch.isnan(loss):
                    ...
                    self.mat_A = torch.tensor(last_lambdas_weight, requires_grad=True)
            ...
            with torch.no_grad():
                ...
                if y_stop is not None:
                    ...
                    if best_stop_loss > epoch_loss_stop.item():
                        ...
                        best_lambdas_weight = self.mat_A
                        best_q = self.q.item()
        ...
        if keep_best:
            ...
            self.mat_A = torch.tensor(best_lambdas_weight, requires_grad=True)
    ...
    def conformalize(self, x_calibration_tensor, y_calibration_tensor, alpha):
        f_x_calibration = self.model(x_calibration_tensor)

        Lambdas_calibration = self.get_Lambdas_whithout_inf(x_calibration_tensor)

        self.nu = get_p_value_mu(y_calibration_tensor, f_x_calibration, Lambdas_calibration, alpha, torch.tensor(self.q) ).item()
        return self.nu

   
    def get_volume_and_coverage(self, x_test_tensor, y_test_tensor):        
        f_x_test = self.model(x_test_tensor)
        
        Lambdas_test = self.get_Lambdas_whithout_inf(x_test_tensor)
    ...
    def get_Lambdas(self, x):
        k = self.mat_A.shape[1]
        one_lambda = self.mat_A @ self.mat_A.T 
        Lambdas = one_lambda.unsqueeze(0).expand(x.shape[0], k, k).clone()
        return Lambdas
