...
class MatrixPredictor(nn.Module):
    ...
    def init_weights(self, m, std):
        if isinstance(m, nn.Linear):
            nn.init.normal_(m.weight, mean=0.0, std=std)  # Loi normale N(0, std)
            if m.bias is not None:
                nn.init.constant_(m.bias, 0)  # Initialisation des biais à 0
...
class Network(nn.Module):
    def __init__(self, input_channels, output_channels, hidden_dim = 10, n_hidden_layers = 1):
        super(Network, self).__init__()
        print("Network with ", n_hidden_layers, "hidden layers and a hidden dimension of ", hidden_dim)
        self.fc1 = nn.Linear(input_channels, hidden_dim)
        self.tab_hidden = nn.ModuleList([nn.Linear(hidden_dim, hidden_dim) for _ in range(n_hidden_layers)])
    ...
    def fit_and_plot(self, train_loader, test_loader, epochs, keep_best = True, lr = 0.001, verbose=False):
        if not keep_best:
...
class ParametricRotationMatrix(nn.Module):
    def __init__(self, input_dim, n, hidden_dim = 100):
        ...
        """
        super(ParametricRotationMatrix, self).__init__()
